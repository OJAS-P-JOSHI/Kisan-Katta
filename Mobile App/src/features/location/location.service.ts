...
import type { LocationDistrict, LocationTaluka, LocationVillage } from './location.types';
...
export const fetchDistricts = async (): Promise<LocationDistrict[]> => {
  ...
  const { data } = await api.get<ApiSuccessResponse<LocationDistrict[]>>(ENDPOINTS.districts);
  districtsCache = data.data;
  return districtsCache;
};

/** GET /api/v1/location/talukas/:districtCode */
export const fetchTalukas = async (districtCode: number): Promise<LocationTaluka[]> => {
...
export const fetchVillages = async (talukaCode: number): Promise<LocationVillage[]> => {
...
/** Test / logout helper — not required by UI today. */
