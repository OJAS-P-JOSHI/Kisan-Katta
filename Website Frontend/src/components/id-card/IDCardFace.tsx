...
/** Display size for QR under photo; generator still renders at 2×. */
const QR_DISPLAY_PX = 120
const PHOTO_COL_PX = 120

/**
 * Pure visual face of the Digital ID — captured for PNG/PDF.
 * Layout polish only — no business/QR-content changes.
 */
export const IDCardFace = forwardRef<HTMLDivElement, IDCardFaceProps>(function IDCardFace(
  ...
  return (
    ...
    >
      ...
      {/* Body: photo + QR | identity fields */}
      <div className="relative z-10 grid gap-5 p-5 sm:grid-cols-[120px_1fr] sm:items-start">
        {/* Subtle watermark */}
        <img
          src={brandAssets.logo}
          alt=""
        ...
        {/* Left column: Photo → QR → Scan label */}
        <div
          className="relative z-10 mx-auto flex flex-col items-center gap-2.5 sm:mx-0"
          style={{ width: PHOTO_COL_PX }}
        >
            ...
            <div className="flex w-full flex-col items-center gap-1.5 pt-1">
              ...
              >
                {qrDataUrl ? (
                  ...
                  <div className="flex h-[120px] w-[120px] items-center justify-center bg-forest-50 text-[10px] text-steel">
                    QR
                  </div>
                )}
              </div>
